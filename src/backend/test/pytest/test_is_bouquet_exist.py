...
module_name = 'is_bouquet_exist'
module_path = os.path.abspath(os.path.join(
    os.path.dirname(__file__), '../..', 'lambda', 'is_bouquet_exist', 'is_bouquet_exist.py'))
...
def test_lambda_handler_can_create_bouquet(mock_boto_resource):
    """Test case where bouquet can be created."""
    ...
    # Mock generative AI table response (5 or more flowers)
    mock_flower_table = MagicMock()
    mock_flower_table.scan.return_value = {
        'Items': [{}] * 5  # Mocking 5 flower entries
    }
    ...
    assert body['can_create_bouquet'] is True
