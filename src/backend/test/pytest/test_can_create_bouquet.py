...
@patch.dict(
    "os.environ",
    {
        "BOUQUET_TABLE_NAME": "TestBouquetTable",
        "GENERATIVE_AI_TABLE_NAME": "TestGenerativeTable",
    },
)
def test_lambda_handler_bouquet_already_created(mock_boto_resource):
    ...
    mock_dynamodb.Table.side_effect = (
        lambda name: mock_bouquet_table if name == "TestBouquetTable" else None
    )
    ...
    mock_dynamodb.Table.side_effect = (
        lambda name: mock_flower_table
        if name == "TestGenerativeTable"
        else mock_bouquet_table
    )
...
def test_lambda_handler_cannot_create_bouquet_no_flowers(mock_boto_resource):
    ...
    mock_dynamodb.Table.side_effect = (
        lambda name: mock_flower_table
        if name == "TestGenerativeAI_TABLE_NAME"
        else mock_bouquet_table
    )
